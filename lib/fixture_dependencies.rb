...
class << FixtureDependencies
  ...
  def get(record)
    ...
    model_method(:model_find, model_type(model), model, fixtures[model_name.to_sym][name.to_sym][pk_hash_key(model.primary_key)])
  end
  ...
  def load_yaml(model_name)
    ...
    yaml.each do |name, attributes|
      ...
      composite_key = attributes.inject([]){|ckey, (k,v)| [model.primary_key].flatten.include?(k.to_sym) ? ckey << v : ckey }
      symbol_attrs[pk_hash_key(model.primary_key)] = composite_key if composite_key.size > 1
  ...
  def use(record, loading = [], procs = {})
    ...
    puts "pk hash key #{pk_hash_key(model.primary_key)}" if verbose > 2
    puts "pk attributes #{attributes[pk_hash_key(model.primary_key)]}" if verbose > 2
    if existing_obj = model_method(:model_find_by_pk, mtype, model, attributes[pk_hash_key(model.primary_key)])
    ...
    attributes.each do |attr, value|
      if reflection = model_method(:reflection, mtype, model, attr.to_sym)
        if [:belongs_to, :many_to_one].include?(model_method(:reflection_type, mtype, reflection))
          ...
          if dep_name == record
            ...
            value = attributes[pk_hash_key(model.primary_key)]
            ...
            (procs[dep_name] ||= []) << Proc.new do |assoc|
              m = model_method(:model_find, mtype, model, attributes[pk_hash_key(model.primary_key)])
              m.send("#{attr}=", assoc)
      ...
      obj.send("#{attr}=", value) unless attr.to_sym == pk_hash_key(model.primary_key) && [model.primary_key].flatten.size > 1
    end
    ...
    fixtures[model_name.to_sym][name.to_sym][pk_hash_key(model.primary_key)] ||= obj[pk_hash_key(model.primary_key)]
  ...
  def pk_hash_key(pk)
    [pk].flatten.inject(""){|a,o| a == "" ? a + o.to_s : a + '_' + o.to_s}.to_sym
  end
end
